...
const gradeBadge = (grade) => {
  ...
  return (
    <Badge bg={s.bg} color={s.color} px="12px" py="4px" borderRadius="12px" fontWeight="700" fontSize="15px" textTransform="none">
      {grade || "—"}
    </Badge>
  );
};
...
const entryStatusBadge = (status) => {
  ...
  return (
    <Badge bg={s.bg} color={s.color} px="8px" py="3px" borderRadius="10px" textTransform="none" fontSize="11px" fontWeight="500">
      {status}
    </Badge>
  );
};
        ...
        <Flex p="16px" justifyContent="space-between" alignItems="center">
          <Flex alignItems="center" gap="10px">
            <Box color="gray.400" mt="1px">
              {open ? <FaChevronDown size="12px" /> : <FaChevronRight size="12px" />}
            </Box>
            <Box>
              <Text fontSize="15px" fontWeight="600" color="#1A202C">{cat.category}</Text>
              <Text fontSize="12px" color="gray.400" mt="1px">Weight: {cat.weight}%</Text>
            </Box>
          </Flex>

          <Flex alignItems="center" gap="20px">
            <Box textAlign="right" minW="120px">
              <Flex alignItems="center" gap="8px" justifyContent="flex-end" mb="4px">
                <Text fontSize="13px" fontWeight="600" color={barColor}>{pct.toFixed(1)}%</Text>
                <Text fontSize="12px" color="gray.400">avg</Text>
              </Flex>
              <Progress value={pct} size="xs" borderRadius="4px" sx={{ "& > div": { background: barColor } }} />
            </Box>
            <Box textAlign="right" minW="80px">
              <Text fontSize="11px" color="gray.400" mb="1px">Contribution</Text>
              <Text fontSize="14px" fontWeight="700" color="#1A202C">{(cat.contribution ?? 0).toFixed(1)} pts</Text>
            </Box>
            ...
            {cat.entries?.length > 0 ? (
              <TableContainer>
                <Table size="sm" variant="simple">
                  <Thead bg="#F7FAFC">
                    <Tr>
                      {["Assessment", "Type", "Score", "Grade", "Feedback", "Status"].map((h) => (
                        <Th key={h} py="10px" color="gray.500" fontSize="11px" fontWeight="600" textTransform="none">{h}</Th>
                      ))}
                    </Tr>
                  </Thead>
                  <Tbody>
                    {cat.entries.map((e) => (
                      <Tr key={e.id} _hover={{ bg: "#FAFAFA" }}>
                        <Td py="10px" fontSize="13px" fontWeight="500">{e.assessmentName}</Td>
                        <Td py="10px" fontSize="12px" color="gray.500" textTransform="capitalize">{e.assessmentType}</Td>
                        <Td py="10px" fontSize="13px">
                          <Text fontWeight="600">{e.score}/{e.maxScore}</Text>
                          <Text fontSize="11px" color="gray.400">{e.maxScore > 0 ? ((e.score / e.maxScore) * 100).toFixed(1) : 0}%</Text>
                        </Td>
                        <Td py="10px">
                          <Badge
                            color={e.grade === "A" ? "#38A169" : e.grade === "F" ? "#E53E3E" : "#3182CE"}
                            bg="transparent"
                            fontWeight="700"
                            fontSize="13px"
                          >
                        ...
                        <Td py="10px" fontSize="12px" color="gray.600" maxW="200px">
              ...
              <Flex justifyContent="center" py="20px">
                <Text fontSize="13px" color="gray.400">No entries yet for this category.</Text>
              </Flex>
            )}
          </Box>
        </Collapse>
...
const MyGradeBookPage = () => {
  ...
  const gradeColorPrimary = finalGrade === "A"
    ? "#38A169"
    : finalGrade === "F"
    ? "#E53E3E"
  ...
  return (
    ...
    >
      ...
      {resource.err && (
        <Flex justifyContent="center" alignItems="center" minH="300px">
          <Text color="red.500">Failed to load your grade book. Please try again.</Text>
        </Flex>
      )}
      ...
      {!resource.loading && !resource.err && data && (
        <>
          ...
          <Box mb="24px">
            ...
            {data.studentName && (
              <Text fontSize="14px" color="gray.500">{data.studentName}</Text>
            )}
          </Box>

          {/* Score summary card */}
          <Box bg="white" border="1px solid #E2E8F0" borderRadius="12px" p="24px" mb="24px">
            <Grid templateColumns={{ base: "1fr", md: "1fr auto" }} gap="24px" alignItems="center">
              {/* Left: overall score */}
              <Box>
                <Text fontSize="13px" color="gray.500" mb="8px" fontWeight="500">Overall Score</Text>
                <Flex alignItems="flex-end" gap="12px" mb="12px">
                  <Text fontSize="48px" fontWeight="800" color={gradeColorPrimary} lineHeight="1">
                    {finalScore.toFixed(1)}
                  </Text>
                  <Text fontSize="20px" color="gray.400" mb="8px">/ 100</Text>
                </Flex>
                <Box>
                  <Flex justifyContent="space-between" mb="6px">
                    <Text fontSize="12px" color="gray.500">Progress</Text>
                    <Text fontSize="12px" fontWeight="600" color={gradeColorPrimary}>{finalScore.toFixed(1)}%</Text>
                  </Flex>
                  <Progress
                    value={finalScore}
                    size="sm"
              ...
              <Flex gap="20px" flexWrap="wrap" justifyContent={{ base: "flex-start", md: "flex-end" }}>
                <Box
                  ...
                  minW="100px"
                >
                  <Text fontSize="11px" color="gray.400" mb="6px" fontWeight="500">Final Grade</Text>
                  <Text fontSize="40px" fontWeight="800" color={gradeColorPrimary} lineHeight="1">
                    {finalGrade || "—"}
                  </Text>
                </Box>
                ...
                <Flex flexDirection="column" gap="12px" justifyContent="center">
                  <Box>
                    <Text fontSize="11px" color="gray.400" fontWeight="500">Calculation</Text>
                    <Text fontSize="14px" fontWeight="600" color="#1A202C" textTransform="capitalize">
                      {(data.calculationMethod || "—").replace("_", " ")}
                    </Text>
                  </Box>
                  <Box>
                    <Text fontSize="11px" color="gray.400" fontWeight="500">Status</Text>
                    <Badge
                      bg={data.status === "published" ? "#E6F4EA" : data.status === "finalized" ? "#EBF4FF" : "#F7FAFC"}
                      color={data.status === "published" ? "#38A169" : data.status === "finalized" ? "#3182CE" : "#718096"}
                      px="10px" py="4px" borderRadius="10px" textTransform="none" fontWeight="500"
                    >
                      {data.status || "draft"}
                    </Badge>
                  </Box>
            ...
            {data.breakdown?.length > 0 && (
              <>
                <Divider my="20px" />
                <Text fontSize="12px" fontWeight="600" color="gray.500" mb="10px">Category Weights</Text>
                <Flex gap="2px" borderRadius="6px" overflow="hidden" h="10px" mb="10px">
                  ...
                  {data.breakdown.map((cat, i) => {
                    ...
                    return (
                      <Flex key={cat.category} alignItems="center" gap="6px">
                        ...
                        <Text fontSize="12px" color="gray.600">{cat.category} ({cat.weight}%)</Text>
                      </Flex>
                    );
                  })}
                </Flex>
          ...
          {/* Category breakdowns */}
          <Box mb="8px">
            <Text fontSize="16px" fontWeight="700" color="#1A202C" mb="14px">Grade Breakdown by Category</Text>
            {data.breakdown?.length > 0 ? (
              data.breakdown.map((cat, i) => (
                <CategoryRow key={cat.category} cat={cat} index={i} />
              ))
            ) : (
              <Box bg="white" border="1px solid #E2E8F0" borderRadius="10px" p="32px" textAlign="center">
