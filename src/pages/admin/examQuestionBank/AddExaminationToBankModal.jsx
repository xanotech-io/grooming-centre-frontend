...
const AddExaminationToBankModal = ({ isOpen, onClose, examinationId, courseId, moduleId, examinationTitle }) => {
  ...
  useEffect(() => {
    ...
    setProgress("");
    setLoading(true);
    requestExaminationDetails(examinationId, true)
      .then(({ examination }) => {
      ...
      .catch((err) => {
        console.error("[AddExaminationToBankModal] failed to load exam questions", err);
        setQuestions([]);
  ...
  const handleCopy = async () => {
    ...
    for (let i = 0; i < questions.length; i += 1) {
      ...
      try {
        ...
        console.error("[AddExaminationToBankModal] failed to add question to bank", payload, err?.response?.data ?? err);
        failCount += 1;
      }
    }
    toast({
...
export default AddExaminationToBankModal;
