...
const TrackingReportingPage = () => {
  ...
  let exportData = [];
  let exportMapRow = () => [];
  let exportFilename = "tracking-report";

  switch (activeTab) {
    case 0:
      exportHeaders = ["Operation ID", "Type", "File Format", "Records", "Source", "Performed By", "Status", "Validation Status", "Date", "Time", "Remarks"];
      exportData = importExportData;
      exportMapRow = (item) => [item.id, item.type, item.format, item.records, item.source, item.performedBy, item.status, item.validation, item.date, item.time, item.remarks];
      exportFilename = "data-import-export-interface";
      break;
    case 1:
      exportHeaders = ["Course ID", "Course Title", "Student", "Student ID", "Enrollment Status", "Enrollment Date", "Instructor", "Remarks"];
      exportData = enrollmentStatusData;
      exportMapRow = (item) => [item.id, item.title, item.student, item.studentId, item.status, item.date, item.instructor, item.remarks];
      exportFilename = "course-enrollment-status-display";
      break;
    case 2:
      exportHeaders = ["Report ID", "Student Name", "Student ID", "Address", "Department", "Enrollment Status", "Generated By", "Timestamp", "Remarks"];
      exportData = reportWorklistData;
      exportMapRow = (item) => [item.id, item.name, item.studentId, item.address, item.dept, item.status, item.generatedBy, item.timestamp, item.remarks];
      exportFilename = "student-report-worklist-generation";
      break;
    case 3:
      exportHeaders = ["Report ID", "Student", "Student ID", "Curriculum Title", "Course Title", "Completion Status", "Assessment Score", "Overall Progress (%)", "Notified By", "Remarks", "Timestamp"];
      exportData = trainingProgressData;
      exportMapRow = (item) => [item.id, item.student, item.studentId, item.curriculum, item.course, item.status, item.score, item.progress, item.notifiedBy, item.remarks, item.timestamp];
      exportFilename = "student-training-progress-report";
      break;
    case 4:
      exportHeaders = ["Report ID", "Course Title", "Course ID", "Department", "Instructor", "Enrollment Count", "Course Capacity", "Enrollment Status", "Date Generated", "Generated By"];
      exportData = courseEnrollmentData;
      exportMapRow = (item) => [item.id, item.title, item.courseId, item.dept, item.instructor, item.count, item.capacity, item.status, item.date, item.generatedBy];
      exportFilename = "course-enrollment-status-report";
      break;
    case 5:
      exportHeaders = ["Notification ID", "Name", "Employee ID", "Training Course Title", "Completion Status", "Compliance Due Date", "Notification Type", "Notification Date", "Notified By", "Remarks"];
      exportData = trainingComplianceData;
      exportMapRow = (item) => [item.id, item.name, item.employeeId, item.course, item.status, item.dueDate, item.type, item.date, item.notifiedBy, item.remarks];
      exportFilename = "training-compliance-notification-report";
      break;
    case 6:
      exportHeaders = ["Verification ID", "Name", "Employee ID", "Course Title", "Enrollment Status", "Action Taken", "Verification Timestamp", "Verified By", "Remarks"];
      exportData = duplicateEnrollmentData;
      exportMapRow = (item) => [item.id, item.name, item.employeeId, item.course, item.status, item.actionTaken, item.timestamp, item.verifiedBy, item.remarks];
      exportFilename = "duplicate-enrollment-verification-report";
      break;
    default:
      break;
  }

  const exportRows = [exportHeaders, ...exportData.map(exportMapRow)];
